 ...
 */
function registerERC1155(IBaseWorld world, bytes14 namespace, string memory uri) returns (IERC1155Mintable token) {
  ...
  world.installModule(erc1155Module, abi.encode(namespace, uri));

  // Return the newly created ERC1155 token
  token = IERC1155Mintable(
    ERC1155Registry.get(ERC1155_REGISTRY_TABLE_ID, WorldResourceIdLib.encodeNamespace(namespace))
  );
}
